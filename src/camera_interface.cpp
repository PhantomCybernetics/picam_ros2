...
bool CameraInterface::initializeEncoder() {
    ...
    /// [use 3/gop]
    this->codec_context->max_b_frames = 3;
...
builtin_interfaces::msg::Time get_current_stamp(std::shared_ptr<PicamROS2> node) {
    rclcpp::Time now = node->get_clock()->now();
    
    // Split into seconds and nanoseconds
    builtin_interfaces::msg::Time stamp;
    stamp.sec = static_cast<int32_t>(now.seconds());
    stamp.nanosec = static_cast<uint32_t>(now.nanoseconds() % 1000000000);
...
void CameraInterface::requestComplete(Request *request) {
    ...
    for (auto bufferPair : buffers) {
        ...
        frame->buf[0] = av_buffer_create(
            ...
            planes[0].length,
            free_buffer,
            reinterpret_cast<void*>(planes[0].length),
            0
        );
        ...
        for (size_t i = 0; i < planes.size(); ++i) {
            ...
            // Map the memory
            std::cout << "Mapping plane #" << i << " fd=" << planes[i].fd.get();
            std::cout << "; off=" << planes[i].offset << "; len=" << planes[i].length;

            // Assign plane data using offsets within the mapped buffer
            frame->data[i] = static_cast<uint8_t*>(base) + planes[i].offset;
            frame->linesize[i] = (i == 0) ? this->streamConfig.stride : this->streamConfig.stride / 2;

            std::cout << "; stride=" << frame->linesize[i];
            std::cout << std::endl;
        }
        ...
        header.stamp = get_current_stamp(this->node);
